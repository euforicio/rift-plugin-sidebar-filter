...
const DIALECTS = {
   ...
   * raw AgentSessionEvent (or an `extension_ui_request`). The rift extension's
   * channel (fd 3 child → bridge, fd 4 bridge → child) is recorded on the
   * same lanes wrapped as `{ riftChannel: <message> }`; this dialect routes
  ...
  "pi-rpc": {
    ...
    classify(message) {
      const channel = message.riftChannel;
      if (typeof channel === "object" && channel !== null) {
        // The extension mints tool-call ids; the bridge mints request ids
        // (`cr-N`), disjoint from its stdin ids (`rift-N`).
    ...
    isInitialize(classified) {
      ...
      // child from `rift-1`; later `get_state` probes (compaction guard, steer
      // settlement) carry higher ids and do not start a segment.
      return (
        classified.kind === "request" &&
        classified.key === "get_state" &&
        classified.id === "rift-1"
      );
    },
    withResponseId(message, id) {
      if (
        typeof message.riftChannel === "object" &&
        message.riftChannel !== null
      ) {
        return { ...message, riftChannel: { ...message.riftChannel, id } };
      }
      return { ...message, id };
    },
    genericResponse(id, classified) {
      if (classified?.channel) {
        return { riftChannel: { kind: "reply", id, result: {} } };
      }
