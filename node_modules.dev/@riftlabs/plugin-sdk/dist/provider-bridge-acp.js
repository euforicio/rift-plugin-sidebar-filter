...
function fetchDashboard(method, accessToken) {
  return fetch(`${CURSOR_DASHBOARD_URL}/${method}`, {
    ...
    headers: {
      ...
      "x-cursor-client-version": "cli-rift-provider-acp"
    },
    body: "{}",
...
function riftToolItem(event, injected) {
  return {
    item: {
      type: "tool",
      tool: injected.name,
      server: RIFT_TOOL_SERVER,
      ...genericToolFields(event)
    },
...
function classifyAcpToolCall2(event, injected, options) {
  if (injected !== void 0 && isInjectedToolCandidate(event)) {
    return riftToolItem(event, injected);
  }
...
var ACP_BRIDGE_MCP_SERVER_NAME = "rift-bridge";
var ENV_HOST = "RIFT_ACP_DYNAMIC_TOOL_HOST";
var ENV_PORT = "RIFT_ACP_DYNAMIC_TOOL_PORT";
var ENV_TOKEN = "RIFT_ACP_DYNAMIC_TOOL_TOKEN";
var ENV_THREAD_ID = "RIFT_ACP_DYNAMIC_TOOL_THREAD_ID";
var ENV_TOOLS = "RIFT_ACP_DYNAMIC_TOOLS";
var ENV_PROGRESS_INTERVAL_MS = "RIFT_ACP_DYNAMIC_TOOL_PROGRESS_INTERVAL_MS";
  ...
  switch (message.method) {
    case "initialize":
      ...
      void callBridge(env, {
        ...
        process.stderr.write(
          `rift-bridge MCP: failed to report initialize: ${error instanceof Error ? error.message : String(error)}
`
        );
      });
      return;
...
async function writeApprovals(path5, approvals) {
  ...
  const tempPath = `${path5}.rift-app-${process.pid}-${randomBytes(6).toString("hex")}.tmp`;
  try {
...
async function acquireApprovalLock(path5) {
  const lockPath = `${path5}.rift-app-lock`;
...
// ../provider-bridge-acp/src/bridge/bridge.ts
var sessionsByRiftThreadId = /* @__PURE__ */ new Map();
var riftThreadIdByProviderThreadId = /* @__PURE__ */ new Map();
...
function emitForSession(session, method, params) {
  sendThreadDeltas(
    session.riftThreadId,
    session.translator.translateAcpEvent(
      { jsonrpc: "2.0", method, params },
      { threadId: session.riftThreadId }
    )
  );
}
...
async function forwardDynamicToolCall(args) {
  const session = sessionsByRiftThreadId.get(args.threadId);
  if (!session || !session.providerThreadId || session.stopping) {
    return { ok: false, error: "No active ACP session for dynamic tool call." };
  }
  session.translator.noteInjectedToolCall(session.riftThreadId, args.tool);
  try {
      ...
      (async () => {
        const initializeResult = await connection.request({
          ...
          params: {
            ...
            clientInfo: { name: "rift", version: "1.0.0" },
...
function handlePermissionRequest(session, params, responder) {
  ...
  const bound = toolCall?.toolCallId !== void 0 ? session.translator.notePermissionToolCall(session.riftThreadId, {
    toolCallId: toolCall.toolCallId,
    ...toolCall.title !== void 0 ? { title: toolCall.title } : {},
    ...toolCall.kind !== void 0 ? { kind: toolCall.kind } : {},
    ...toolCall.rawKind !== void 0 ? { rawKind: toolCall.rawKind } : {},
...
async function handleFsWriteTextFile(session, params, responder) {
  ...
  if (session.policy.permissionMode === "accept-edits" && !isPathInsideRoots(parsed.data.path, session.policy.workspaceWriteRoots)) {
    responder.error(
      -32e3,
      `File writes outside the workspace are denied by RIFT's accept-edits permission mode: ${parsed.data.path}`
    );
    return;
  }
  try {
...
function liveSessionForThread(riftThreadId) {
  const session = sessionsByRiftThreadId.get(riftThreadId);
...
function removeSession(session) {
  if (sessionsByRiftThreadId.get(session.riftThreadId) === session) {
    sessionsByRiftThreadId.delete(session.riftThreadId);
  }
  if (riftThreadIdByProviderThreadId.get(session.providerThreadId) === session.riftThreadId) {
    riftThreadIdByProviderThreadId.delete(session.providerThreadId);
  }
}
async function releaseCursorMcpApproval(session) {
  ...
  try {
    ...
    process.stderr.write(
      `acp bridge: failed to remove Cursor session MCP approval for thread "${session.riftThreadId}": ${error instanceof Error ? error.message : String(error)}
`
    );
  }
}
function getSessionByProviderThreadId(providerThreadId) {
  const riftThreadId = riftThreadIdByProviderThreadId.get(providerThreadId);
  return riftThreadId ? sessionsByRiftThreadId.get(riftThreadId) : void 0;
}
async function startAgentSession(request) {
  const params = request.params;
  const riftThreadId = params.threadId;
  const existing = sessionsByRiftThreadId.get(riftThreadId);
  if (existing) {
  ...
  const connection = createAcpAgentConnection({
    ...
    onExit: (info) => {
      const wasCurrent = sessionsByRiftThreadId.get(riftThreadId) === session;
  ...
  sessionsByRiftThreadId.set(riftThreadId, session);
  try {
    const initializeResult = await connection.request({
      method: "initialize",
      params: {
    ...
    if (mcpServer) {
      ...
      if (session.cursorMcpApproval?.installedByRift) {
        process.stderr.write(
          `acp bridge: installed Cursor session MCP approval for thread "${riftThreadId}"
`
        );
      }
    }
    ...
    if (session.stopping) {
      throw new Error(
        `ACP session for thread "${riftThreadId}" was released during construction`
      );
    }
    session.providerThreadId = sessionId;
    riftThreadIdByProviderThreadId.set(sessionId, riftThreadId);
    sendNotification(BRIDGE_NOTIFICATION_METHODS.threadIdentity, {
      threadId: riftThreadId,
      providerThreadId: sessionId,
      sessionRestorable: session.supportsLoadSession
    });
    sendThreadDeltas(riftThreadId, [{ kind: "session.reset" }]);
...
async function handleRequest2(request) {
  switch (request.method) {
    ...
    case "thread/stop": {
      const session = sessionsByRiftThreadId.get(request.params.threadId);
      if (session) {
...
async function stopAllSessions() {
  await Promise.all(
    Array.from(sessionsByRiftThreadId.values()).map(
...
async function probeAcpAgent(request) {
  ...
  try {
    connection = createAcpAgentConnection({
      ...
      onRequest: (_method, _params, responder) => {
        responder.error(-32601, "rift is probing this agent's capabilities");
      },
      onExit: () => {
      }
    });
