...
declare const skillListResponseSchema: z$1.ZodObject<{
    skills: z$1.ZodArray<z$1.ZodObject<{
        ...
        scope: z$1.ZodEnum<{
            ...
            "rift-builtin": "rift-builtin";
            "rift-project": "rift-project";
            "rift-user": "rift-user";
...
declare const pluginSourceDetailSchema: z$1.ZodObject<{
    engines: z$1.ZodObject<{
        rift: z$1.ZodOptional<z$1.ZodString>;
        riftPluginSdk: z$1.ZodOptional<z$1.ZodString>;
    }, z$1.core.$strip>;
 ...
 * the same without becoming part of RIFT's public protocol.
 */
...
/**
 * The `@riftlabs/plugin-sdk/app` contract (plugin design §5.2) — pure types with no
 * side effects. The RIFT app imports these to keep its real implementation in
...
interface PluginNavPanelProps {
     ...
     * `useRiftNavigate().toPluginPanel(path, { subPath })` — browser
...
interface PluginThreadListProps {
    ...
    onNavigate: () => void;
    /**
     * Compatibility value for the former sidebar search field. RIFT now searches
    ...
    searchQuery: string;
    /**
     * RIFT's thread list, bound to this sidebar instance. Render it to delegate
    ...
    /** @deprecated Renamed to `Original` in SDK 0.4.16; removed in rift 0.42. */
    experimental_Original?: ComponentType;
}
/**
...
interface PluginFileOpenerProps {
    ...
    /**
     * RIFT's file preview, bound to this file. Render it to delegate conditionally
...
/**
 * Props of the host-owned `experimental_SourceCode` component — RIFT's source
 * viewer. The host owns syntax highlighting, gutters, wrapping, line-selection
 * presentation, and the live RIFT code theme; the caller owns loading the text
...
/**
 * Props of the host-owned `experimental_Diff` component — RIFT's diff viewer.
 ...
 * and the live RIFT code theme. Content that cannot be parsed as a patch
...
interface DiffProps {
     ...
     * patch, RIFT enables expand-context controls between hunks. The caller owns
...
interface PluginSourceCodeRendererProps {
    ...
    highlightedLines: SourceCodeLineRange | null;
    /**
     * RIFT's source renderer, bound to this request. Render it to delegate
     * conditionally without re-entering plugin replacement resolution.
     *
     * @experimental Audit before relying on this as a stable contract.
     */
...
interface PluginDiffRendererProps {
     ...
     * treating the contents as complete. RIFT's original renderer performs that
     * verification when it mounts.
     */
    experimental_fullFileContents: ExperimentalDiffFullFileContents | null;
    /**
     * RIFT's diff renderer, bound to this request. Render it to delegate
     * conditionally without re-entering plugin replacement resolution.
     *
     * @experimental Audit before relying on this as a stable contract.
     */
...
/**
 * Render app-wide plugin UI outside RIFT's layout regions.
 *
 ...
 * or creates a React portal. RIFT supplies no chrome, positioning, visibility,
...
type PluginFixedTabRegistration<Target extends JsonValue = never> = ExperimentalPluginFixedTabReference<Target> & {
    title: string;
    /** Icon hint (RIFT icon name); unknown names fall back to a generic icon. */
    icon: string;
...
interface PluginNavPanelRegistration {
     ...
     * RIFT owns selection and persistence and always includes its native Browser
...
interface PluginThreadPanelActionContext {
     ...
     * `useRiftNavigate().openThreadPanel` (which decline for more reasons) so one
...
interface PluginThreadPanelActionRegistration {
    ...
    title: string;
    /**
     * Icon hint (RIFT icon name) used when the plugin ships no logo; the
...
interface PluginNewThreadPanelActionRegistration {
    ...
    /** Label of the action row in the panel's new-tab launcher. */
    title: string;
    /** Icon hint (RIFT icon name) used when the plugin ships no logo. */
    icon?: string;
    /** Rendered inside every panel tab this action opens. */
...
interface PluginPendingInteractionRegistration {
     ...
     * `rendererId` a backend passes to `rift.ui.requestInput`, and the `<name>`
     ...
     * `rift.ui.requestInput` validates `rendererId` against `/^[a-zA-Z0-9_-]+$/`;
     * an extension kind must match `/^[a-z0-9-]+\/[a-z0-9-]+$/`
     * (`EXTENSION_KIND_PATTERN` in @riftlabs/domain), so an id addressable both ways
...
interface ExperimentalSidebarFooterItemBase {
    ...
    label: string;
    /** RIFT icon-name hint; unknown names fall back to a generic icon. */
    icon: string;
}
...
/**
 * The one status rift would paint for a thread, already resolved through the
 ...
 *
 * Treat an unrecognized value as "none": rift adds kinds over time, and an
...
/**
 * How a thread's environment presents its workspace: a worktree rift manages,
 ...
 * needs. `attention` is rift's rolled-up "does this need you" signal, so a row
...
/**
 * The active code theme as a VS Code theme file: the same document RIFT's own
...
interface PluginCodeThemeData {
    /** Registered theme name — a bundled Shiki name or a RIFT-registered id. */
    name: string;
    type: "dark" | "light";
...
/**
 * The code theme RIFT is currently rendering with (see
...
interface PluginSidebarThreadActions {
    /**
     * Navigate to a thread. `split: true` applies rift's split placement rules —
    ...
    /**
     * Opens rift's delete confirmation, which counts child threads first. Deletion
 ...
 *
 * The frontend sibling of the backend `rift.ui.registerThreadAction`, which
 ...
 * RIFT's list or a specific provider under Settings → Appearance. A plugin can
 * also use its own setting and render `Original` conditionally.
 * An absent or crashing replacement falls back to RIFT's list rather than
 ...
 * order. The user can pin RIFT's preview or a specific opener per extension
...
/**
 * Replace RIFT's source-code renderer everywhere it renders supplied source
 ...
 * order wins. A missing, disabled, or crashing replacement falls back to RIFT's
...
/**
 * Replace RIFT's diff renderer everywhere it renders supplied diff content — the
 ...
 * `messageAction`'s `run`, or any component via `useRiftNavigate()`. A panel
...
interface PluginMessageActionContext {
     ...
     * `useRiftNavigate().openThreadPanel`.
     *
     * Returns true when the host accepted the open; false when it declined —
...
/**
 * A row in rift's quick palette (Mod+Shift+P), listed under the plugin's name
 * beside rift's own commands. Host-rendered: the plugin supplies a title and
...
/**
 * Supply the inline React mark rift draws for one agent provider.
 *
...
interface PluginProviderIconRegistration {
    /**
     * The provider this mark is for — the id rift knows the provider by (the
...
interface PluginAppSlots {
    ...
    /**
     * Replace RIFT's source-code renderer (see
    ...
    /**
     * Replace RIFT's diff renderer (see
...
/** Stable lifecycle values for one content-script instance in one rift client. */
 ...
 * generation in each rift app window or browser tab.
 */
interface PluginContentScriptRegistration {
    /** Unique within the plugin; letters, digits, `-`, `_`. */
    id: string;
    /**
     * Install behavior into the rift app shell. The host awaits a returned
...
/** State of the app's shared realtime connection to the rift server. */
...
interface ComposerPlusMenuItem {
    id: string;
    label: string;
    /** RIFT icon name; unknown names fall back to the generic plugin icon. */
    icon?: string;
...
interface PluginComposerThreadRowStatus {
    /** RIFT icon-name hint; unknown names fall back to the generic plugin icon. */
    icon: string;
...
interface PluginComposerMention {
    /** Mention provider id registered by THIS plugin via `rift.ui.registerMentionProvider`. */
    provider: string;
...
interface PluginComposerApi {
     ...
     * Failures of the underlying request are reported by rift's own submit error
 ...
 * engine), rendered by the RIFT app inside a plugin slot. This is the
 ...
 * forwarded verbatim to `rift.sdk.threads.spawn`, so it must name a real
 * provider and model.
 */
 ...
 * empty catalogs leave `value` unchanged. Omit `routing` to use rift's
...
/** Props of RIFT's controlled, host-resolved permission-mode picker. */
 ...
 * `rift.sdk.threads.spawn`.
 *
 * The split is deliberate: the composer owns *user selections*, the plugin
 * owns *filing and attribution*. `rift.sdk.threads.spawn` auto-fills
...
interface NewThreadRequest {
    /**
     * The selected project id. Choosing "Don't work in a project" submits RIFT's
     ...
     * `rift.sdk.projects.list({ includePersonal: true })`.
     */
    projectId: string;
    providerId: string;
    model: string;
...
/**
 * Props of the host-owned `experimental_NewThreadComposer` component — rift's
 ...
 * rendered by the RIFT app inside a plugin slot.
 *
...
/**
 * Props of the host-owned `Markdown` component — rift's chat message renderer
...
/**
 * Props for RIFT's semantic URL link. The host owns ordinary activation while
...
/** Options shared by RIFT's preview and preferred-external file intents. */
interface ExperimentalFileOpenOptions {
    target: ExperimentalLiveFileTarget;
    location: ExperimentalFileLocation | null;
}
/**
 * Props for RIFT's host-rendered semantic file link. Valid targets receive a
...
interface RiftNavigate {
    ...
    /**
     * Open an HTTP(S) URL using this client's RIFT browser preference. Returns
     * false for schemes the host does not own. Experimental: see
     * docs/api_to_audit.md.
     */
    openUrl(url: string): boolean;
    /** Open a live file in this surface's shared RIFT preview panel. */
...
/**
 * Everything `@riftlabs/plugin-sdk/app` resolves to at runtime. The RIFT app builds
 * the real implementation and `satisfies` this interface; `rift plugin build`
 * shims the specifier to that object on `globalThis.__riftPluginRuntime`.
 */
interface PluginSdkApp {
    ...
    useRiftContext(): RiftContext;
    useRiftNavigate(): RiftNavigate;
     ...
     * engine of its own and needs RIFT's palette to reach it. Experimental: see
     * docs/api_to_audit.md.
     */
    ...
    /**
     * A real anchor whose ordinary HTTP(S) activation uses RIFT's URL preference.
     * Experimental: see docs/api_to_audit.md.
     */
    ...
    /**
     * RIFT's controlled provider/model/reasoning picker. Provider changes emit
    ...
    /**
     * RIFT's controlled permission-mode picker. The host resolves provider
     ...
     * supplied source text with RIFT's syntax highlighting, gutters, and live code
     ...
     * content with RIFT's normalization, optional full-file context expansion,
...
type RiftRealtimeUnsubscribe = () => void;
type RiftRealtimeEventName = "environment:changed" | "host:changed" | "project:changed" | "realtime:connection" | "system:changed" | "system:config-changed" | "thread:changed";
...
type RiftRealtimeConnectionState = "connected" | "connecting" | "disconnected";
interface RiftRealtimeConnectionEvent {
    reconnectDelayMs: number | null;
    reconnected: boolean;
    state: RiftRealtimeConnectionState;
}
interface RiftRealtimeEventMap {
    ...
    "realtime:connection": RiftRealtimeConnectionEvent;
}
type RiftRealtimeCallback<TEventName extends RiftRealtimeEventName> = (event: RiftRealtimeEventMap[TEventName]) => void;
interface ThreadRealtimeSubscribeArgs {
    callback: RiftRealtimeCallback<"thread:changed">;
    event: "thread:changed";
    threadId?: string;
}
interface ProjectRealtimeSubscribeArgs {
    callback: RiftRealtimeCallback<"project:changed">;
    event: "project:changed";
    projectId?: string;
}
interface EnvironmentRealtimeSubscribeArgs {
    callback: RiftRealtimeCallback<"environment:changed">;
    environmentId?: string;
    event: "environment:changed";
}
interface HostRealtimeSubscribeArgs {
    callback: RiftRealtimeCallback<"host:changed">;
    event: "host:changed";
    hostId?: string;
}
interface SystemRealtimeSubscribeArgs {
    callback: RiftRealtimeCallback<"system:changed">;
    event: "system:changed";
}
interface SystemConfigRealtimeSubscribeArgs {
    callback: RiftRealtimeCallback<"system:config-changed">;
    event: "system:config-changed";
}
interface RealtimeConnectionSubscribeArgs {
    callback: RiftRealtimeCallback<"realtime:connection">;
    event: "realtime:connection";
}
type RiftRealtimeSubscribeArgsUnion = ThreadRealtimeSubscribeArgs | ProjectRealtimeSubscribeArgs | EnvironmentRealtimeSubscribeArgs | HostRealtimeSubscribeArgs | SystemRealtimeSubscribeArgs | SystemConfigRealtimeSubscribeArgs | RealtimeConnectionSubscribeArgs;
type RiftRealtimeSubscribeArgs<TEventName extends RiftRealtimeEventName = RiftRealtimeEventName> = Extract<RiftRealtimeSubscribeArgsUnion, {
    event: TEventName;
}>;
interface RiftRealtime {
    subscribe<TEventName extends RiftRealtimeEventName>(args: RiftRealtimeSubscribeArgs<TEventName>): RiftRealtimeUnsubscribe;
}
 ...
 * queued row in the workspace, which is what `rift thread queue list` with no
...
interface RiftSdkAreas extends RiftRealtime {
...
interface RiftSdk extends RiftSdkAreas {
    guide: GuideArea;
}
...
/** Define the single host executable exported by `rift.host`. */
...
/**
 * The backend plugin API contract — the `rift` object handed to a plugin's
 * `server.ts` factory (`export default function plugin(rift: RiftPluginApi)`).
 *
 * Types only: the implementation lives in the RIFT server
 ...
 * import them type-only (`import type { RiftPluginApi } from
 * "@riftlabs/plugin-sdk"`); the import is erased when RIFT loads the file.
 *
...
interface PluginStorage {
    /** Namespaced JSON key-value rows in rift.db; values ≤256KB each. */
    kv: PluginKvStorage;
    /**
     ...
     * `_rift_migrations` table; unapplied statements run in one transaction. The
 ...
 * (`rift.sdk.threads.retry`), so the id is the whole of what a policy needs, and
 * anything else about the thread is one `rift.sdk.threads.get` away and fresher
...
interface PluginTurnFailedEvent {
     ...
     * `rift.sdk.threads.retry` takes as `turnRequestId`. On a retry's failure this
...
/**
 * Lifecycle events a plugin can observe with `rift.events.on` (design §4.5).
 *
 ...
 * The surface that *can* is `rift.experimental_hooks`, where core asks a
...
interface PluginThreadEventPayloads {
     ...
     * `entry.waitingOn?.kind === "plugin" && entry.waitingOn.pluginId === rift.pluginId`.
     *
     ...
     * `rift.sdk.threads.retry({ threadId, turnRequestId, sendAt })`. That retry is
...
interface PluginHooks {
     ...
     * `rift.events`, whose handlers are told what already happened and whose
...
interface PluginHttp {
     ...
     * - "local": Origin/Host must be a local RIFT app origin; non-GET requires
     *   content-type application/json (forces a CORS preflight).
     * - "token": requires the per-plugin token (`rift plugin token <id>`) via
     *   the x-rift-plugin-token header or ?token=.
...
interface PluginCliRegistration {
    /** Preferred top-level command name (`rift <name> …`): lowercase [a-z0-9-]+.
     * A core collision logs an activation warning and remains available through
     * `rift plugin run <plugin-id>`. */
    name: string;
    summary: string;
...
interface PluginCli {
    /**
     * Register this plugin's `rift` subcommand. One registration per factory
     * execution; a repeated call is rejected. Core rift commands always win
...
/** Per-turn context handed to rift.agents context providers (design §4.4). */
...
interface PluginAgentToolPresentation {
     ...
     * an entry of the manifest's `rift.branding.experimental_icons` map). A
...
interface PluginAgentToolRegistrationBase {
     ...
     * RIFT shows the standard tool name (`Running <name>` / `Ran <name>`) and the
     * plugin's branding glyph. Approval, error, and interruption states keep
     * RIFT's standard rendering. See docs/api_to_audit.md.
     */
...
interface PluginAgentConfigurationContext {
    ...
    provider: {
        ...
        capabilities: {
            /**
             * The provider ships its own user-question affordance and rift routes it
...
/**
 * Permission modes a provider can run a session in — RIFT's own permission
 ...
 * slash-command typeahead is universal — RIFT injects skills into every
...
interface PluginProviderCapabilities {
    ...
    /** The provider keeps its own thread archive, so RIFT mirrors archive and
     * unarchive onto it instead of tracking the state only in rift's own rows. */
    supportsThreadArchive: boolean;
    /** The provider stores a thread name of its own, so RIFT forwards renames to
     * it. */
 ...
 * `ProviderStrings` in `@riftlabs/domain`, which is the client projection.
 */
...
interface PluginProviderOptionsContext {
    ...
    model: string;
    /** RIFT's permission mode for this command (already clamped to the host). */
     ...
     * mode is a RIFT prompt mode, so the bridge maps it onto whatever the agent
     * calls it natively.
     */
    promptMode?: "plan";
    /**
     * This plugin's own settings values (`rift.settings.define`), read at call
...
/**
 * One provider this plugin contributes to RIFT's provider registry.
 *
 ...
 * `experimental_providerBridge` export of the `rift.host` artifact the manifest
 * names (`PROVIDER_BRIDGE_EXPORT_NAME` in the bridge kit), built into the
 * artifact RIFT ships to hosts. Declaring a provider in a plugin with no
 * `rift.host` entry is refused, because the picker entry would exist and no
 * turn on it could ever run; a `rift.host` entry whose artifact failed to
...
interface PluginProviderDeclaration {
     ...
     * `rift.branding.icon` takes — or, unlike `rift.branding.icon`, one of this
     * plugin's declared icons by its namespaced glyph (`"<pluginId>/<name>"`,
     * an entry of the manifest's `rift.branding.experimental_icons` map; the
    ...
    models?: {
         ...
         * agent state and ignores the workspace path — so rift probes once per host
         * and reuses the answer for every environment on it. `"workspace"` (the
         * default) means project configuration can change the answer, so rift
     ...
     * processes are spawned with every inherited `RIFT_*` variable stripped, so a
     ...
     * `.cursor/skills`, say — names them here so rift can list them beside its
     ...
     * the same two-sided shape as `experimental_nativeSkillRoots`. rift offers
     * them in the composer beside the agent's skills.
     */
    experimental_nativeCommandRoots?: PluginProviderNativeRoots;
    /**
     * This plugin's `rift.host` entry implements
     * `experimental_nativeRootsHostContract` (`@riftlabs/plugin-sdk/host`): core
...
interface PluginAgents {
     ...
     * next constructed. Skill changes follow RIFT's environment runtime policy:
 ...
 * registration; `rift.agents` keeps `configure`, `registerTool`, and
...
interface PluginServerApi {
    /**
     * The operator-configured public app URL from `RIFT_APP_URL`, or `null` when
     * the operator has not configured one. This value is not bind-gated.
     */
    readonly experimental_appUrl: string | null;
    /**
     * This RIFT server's own loopback base URL (e.g. "http://127.0.0.1:38886"),
     ...
     * `rift.sdk`: reading it before the server is listening throws, so prefer
     * reading it from handlers, services, and timers.
     */
    readonly loopbackBaseUrl: string;
    /**
     * This server's data directory — the one holding `config.json`, `rift.db` and
     * `plugins/<id>/`. A plugin cannot compute it: a dev server derives it from
     * its repo root and instance id, so a plugin that guesses `~/.rift-app` reads the
     * production file while the dev server reads another one.
     *
     * For reading rift-managed files a plugin is migrating away from. A plugin's
     * OWN storage is `rift.storage`, which is scoped for it; this is deliberately
     * not a place to write.
     */
    readonly experimental_dataDir: string;
}
/**
 * What a plugin's AI service does. `inference` answers rift's server-side helper
...
/**
 * An AI service a plugin offers from its `rift.host` entry, which implements
 * `experimental_aiServicesHostContract` (`@riftlabs/plugin-sdk/ai-services`).
 * The user selects it with `RIFT_INFERENCE` / `RIFT_TRANSCRIPTION` set to
...
interface PluginAiServices {
     ...
     * plugin must declare a `rift.host` entry; registering without one fails the
...
interface PluginSharedPortTunnelIdentity {
    ...
    label: string;
    /** Gate apex without a scheme, e.g. "riftlabs.app". */
    baseDomain: string;
}
interface PluginHosts {
    /** Create the owning plugin's typed client for its singular `rift.host` entry. */
...
interface PluginStatusApi {
     ...
     * `rift plugin list` and the UI) instead of failing — e.g. a factory or
     ...
     * `rift plugin reload <id>` after configuring.
     */
    needsConfiguration(message: string): void;
}
/**
 * The API object handed to a plugin's factory (design §4). Implemented by
 * the RIFT server; this contract is what plugin `server.ts` files compile
 * against.
 */
interface RiftPluginApi {
    ...
    /** Agent-facing `rift` CLI subcommand (design §4.4). */
    ...
    /**
     * AI services this plugin serves from its `rift.host` entry (helper
     * inference, voice transcription). See `@riftlabs/plugin-sdk/ai-services`.
     */
    readonly experimental_aiServices: PluginAiServices;
    /**
     * The full RIFT SDK, bound to this server over loopback (design §4.1).
