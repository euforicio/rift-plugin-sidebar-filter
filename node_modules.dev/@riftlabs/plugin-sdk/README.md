...
The typed facade RIFT plugin authors compile against. The root preserves the
complete `RiftPluginApi` and `RiftSdk` contract; `./app` is the frontend runtime
that `rift plugin build` replaces with RIFT's shared implementation.
...
the built-in `rift-plugin-authoring` skill synchronized with those declarations.
...
newly opened page cannot control RIFT by accident. The frontend harness records
...
thread-storage file. Ordinary activation opens the shared RIFT preview and its
...
app-wide floating React UI. RIFT mounts the component once per app window through
...
bundled TypeScript/JavaScript that enhances the rift app shell without rendering
...
it is not an app-wide stylesheet hook. Use manifest `rift.themes` entries for
...
for `@riftlabs/plugin-sdk/testing` and `@riftlabs/plugin-sdk/testing/app`; neither subpath
imports RIFT workspace packages or source TypeScript at runtime. Install the SDK
...
```sh
npm install --save-dev @riftlabs/plugin-sdk vitest better-sqlite3 zod cron-parser hono
...
and disposal order. HTTP runs through Hono but does not enforce RIFT's local or
...
temporary directory, secrets are kept in memory, `rift.sdk` is always bound and
...
scope, quote, mention, focus, and clear behavior. It does not reproduce RIFT
layout, CSS, persistence, routing, host authentication, crash boundaries, or
multi-plugin arbitration; use a live RIFT test for those boundaries.

## Declaration surface

The complete root declaration flattens the unpublished RIFT workspace contracts.
The testing declarations reuse that public `@riftlabs/plugin-sdk` root instead of
embedding a second copy, and no declaration depends on unpublished `@riftlabs/*`
...
`rift plugin new` pins it exactly in `devDependencies` — and read the root/app
declarations straight from `node_modules/@riftlabs/plugin-sdk/bundled-types/`,
...
`@riftlabs/plugin-sdk` onto them through their `tsconfig.json`; `rift plugin
types` keeps those refreshed until they migrate.

Generate declarations with `pnpm exec turbo run build:types --filter=@riftlabs/plugin-sdk`
from the repository root. Each bundle creates a TypeScript program, so generation
runs at most two workers and waits for each worker to exit before starting its
replacement. This bounds concurrent compiler heaps on machines with many CPU
cores. Outputs remain in manifest order, use atomic replacement, and are only
rewritten when their content changes. The generated `bundled-types/` directory
is not committed.
